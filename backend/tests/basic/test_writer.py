...
def test_writer_receives_authoritative_application_date_when_supplied() -> None:
    model = ScriptedModel(["报告\n\n1 结论\n\n内容 [[source:1]]。"])
...
def test_writer_fault_timeout_preserves_usage_from_empty_first_response():
    class EmptyThenHangs:
        def __init__(self):
            self.calls = 0
    ...
    outcome = _run(
        WriterAgent(EmptyThenHangs(), call_timeout_seconds=0.01).awrite(
            question="问题",
            context=CONTEXT,
            sources=["https://example.com/a"],
            language="zh-CN",
