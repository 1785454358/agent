...
def test_collect_reuses_initial_search_for_original_question() -> None:
    service, search, fetcher = make_service({"generated": ["https://e.test/generated"]})
    initial = {
        "ok": True,
        "query": "root",
        "results": [{"title": "root", "url": "https://e.test/root", "snippet": ""}],
    }
...
def test_exhausted_tool_limit_starts_no_initial_or_parallel_search() -> None:
    ...
    assert search.calls == []
    assert initial["error"]["code"] == "tool_call_limit"
    assert context == ""
    assert documents == {}
    assert sources == []
    assert results[0].errors == ["tool_call_limit"]


def test_async_search_has_an_individual_fault_timeout() -> None:
    ...
    async def run() -> tuple[dict[str, Any], float]:
        settings = make_settings(tool_timeout_seconds=0.02)
...
def test_fetch_has_an_individual_fault_timeout() -> None:
    async def run():
        settings = make_settings(tool_timeout_seconds=0.02)
        budget = GlobalBudget(settings, datetime.now(UTC))
    ...
    assert documents == {}
    assert sources == []
    assert results[0].errors == ["tool_timeout"]


def test_context_filter_has_an_individual_fault_timeout() -> None:
