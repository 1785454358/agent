...
from deeptrace.basic.research import QueryResearchResult
...
class _DelayedPlanner(_Planner):
    async def aplan(self, *args):
        await asyncio.sleep(0.03)
        return await super().aplan(*args)


class _DelayedWriter(_Writer):
    async def awrite(self, **kwargs):
        await asyncio.sleep(0.03)
        return await super().awrite(**kwargs)
...
def test_plan_node_uses_original_question_when_tool_limit_reached() -> None:
...
def test_plan_node_ignores_old_global_deadline() -> None:
    ...
    nodes.planner = _DelayedPlanner()
    nodes.budget = GlobalBudget(budget_settings, datetime.now(UTC))

    update = asyncio.run(asyncio.wait_for(nodes.plan_node(_state()), timeout=0.2))

    assert update["search_queries"] == ["技术进展", "问题"]
    assert update["events"][-1].event_type == "planning.completed"
...
def test_parallel_research_node_emits_query_events() -> None:
    ...
    update = asyncio.run(
        nodes.parallel_research_node({**_state(), "search_queries": ["a", "b"]})
    )
...
def test_writer_node_still_runs_after_tool_limit() -> None:
    nodes, _collector, _planner = _nodes()
    nodes.budget = _ExpiredBudget()
    writer = nodes.writer
    ...
    assert update["termination_reason"] == "tool_call_limit"
    assert update["final_answer"] == "# 报告"


def test_writer_node_ignores_old_global_deadline() -> None:
    nodes, _collector, _planner = _nodes()
    budget_settings = SimpleNamespace(
        max_runtime_seconds=0.02,
        max_fetched_pages=20,
