...
from deeptrace.basic.planner import PlannerAgent, normalize_question
...
from deeptrace.observability import GlobalBudget, elapsed_seconds, estimate_usage_cost
from deeptrace.writer import WriterAgent
from deeptrace.basic.research import (
...
class ResearchWorkflowNodes:
    ...
    async def plan_node(self, state: GraphState) -> dict[str, Any]:
        ...
        if budget_reason is not None:
            ...
            queries, usage, used_fallback, error = await self.planner.aplan(
                question, initial_results
            )
        events.append(
            self._event(
                "planning.completed",
    ...
    async def parallel_research_node(self, state: GraphState) -> dict[str, Any]:
        ...
        return {
            ...
            "stage_seconds": {"parallel_research": time.perf_counter() - started},
            "step_count": 1,
        }
    ...
    async def writer_node(self, state: GraphState) -> dict[str, Any]:
        ...
        # Research limits never consume the Writer's own request timeout.
        ...
        events.append(
            self._event(
                ...
                details={
                    ...
                    "tool_calls": getattr(self.budget, "tool_calls", 0),
                },
            )
        )
        return {
