...
SearchFunction = Callable[[ToolContext, str, int, set[int] | None], Any]
...
class ParallelResearchService:
    ...
    async def acollect(
        ...
        if (
            ...
            and self._budget.stop_reason(datetime.now(UTC)) is not None
        ):
            results = [
                QueryResearchResult(
                    query=query,
                    ...
                    errors=[self._budget.reason or "tool_call_limit"],
                )
        ...
        for query_index, payload in enumerate(search_payloads):
            ...
            for item in raw_results:
                ...
                raw_content = (
                    item.get("raw_content") if isinstance(item, Mapping) else None
                )
    ...
    async def _get_context(self, query: str, documents: Sequence[RawDocument]) -> str:
        if not documents:
            return ""
        ...
        return await asyncio.wait_for(
            operation, timeout=getattr(self._settings, "tool_timeout_seconds", 45)
        )
    ...
    async def _search_one(self, query: str) -> JsonObject:
        timeout = getattr(self._settings, "tool_timeout_seconds", 45)
        if self._budget is not None and not await self._budget.acquire_tool():
            return _tool_error(self._budget.reason or "tool_call_limit")
        try:
            ...
            return _tool_error("tool_timeout")
        except Exception:
            return {
                "ok": False,
                "error": {
    ...
    async def _fetch_one(self, url: str) -> RawDocument:
        async with self._fetch_semaphore:
            if self._budget is not None and not await self._budget.acquire_tool():
                raise WebFetchError("tool_call_limit", "达到研究工具调用上限")
            return await asyncio.wait_for(
    ...
    def _document_from_search_result(
        ...
        return RawDocument(
            ...
            title=(title.strip() if isinstance(title, str) else "") or normalized_url,
            content=content,
