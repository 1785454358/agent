...
- Supervisor 使用普通工具绑定，不传强制 `tool_choice`；可行动阶段的无效结构最多修复一次，已经由硬边界决定结果时不再调用 Provider。
...
## 2026-09-06 终止短路与报告结构修正

- 运行 `68891660c219` 的第二次重规划已经没有可派发的后续主管轮次，但旧实现仍调用两次 Supervisor，并因 `invalid_arguments` 降级。本次将无缺口、研究员上限、网络额度不足和主管轮次上限判断移到 Provider 边界之前。
- 仍有叶子缺口且具备完整补查容量时继续调用 Supervisor；可行动阶段的结构修复、超时熔断和确定性缺口计划保持不变。
- 中文 Writer 契约要求开头使用“1 总述”，主题章节先给出概括或承接内容，存在实质缺口时才写“研究局限”，并以“综合结论”作为正文最后一节。
