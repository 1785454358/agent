...
### 25. 为什么长期记忆同时使用 MySQL 和 Chroma

Distributed 模式用 MySQL 保存作用域、类型、状态、版本、置信度、重要度和来源等权威结构化数据，并负责事务更新。Chroma 根据当前问题找到语义相近的候选。系统先用 MySQL 限定合法候选，再让 Chroma 在候选 ID 内做 TopK，最后回查 MySQL，因此向量索引无法绕过权限和生命周期规则。Local 使用进程内 Memory Store 代替 MySQL，API 重启后不保留结构化长期记忆。

### 26. Chroma 与 MySQL 不一致时怎样处理

写入顺序以 MySQL 为先。Chroma 失败不会回滚已经确认的长期记忆。召回前会比较内容哈希并补齐缺失或变化的索引。Chroma 查询异常时退回确定性关键词排序。旧索引即使暂时存在，也无法进入 MySQL 提供的 candidate_ids。

### 27. 为什么选择 BGE-M3

项目需要处理中英文研究问题，本机已有 BAAI/bge-m3，可以离线生成统一的 1024 维向量，避免把记忆正文交给外部 embedding 服务。模型在执行进程内懒加载一次，编码放到线程执行并用锁限制并发。Local 由 API 进程加载，Distributed 只由 Worker 加载，API 不挂载模型。
...
长期记忆自动清扫尚未调度，Local 的结构化长期记忆还没有持久化，短期窗口按消息数而非精确 Token，API 仍是固定身份的单租户。Chroma 使用单集合和候选 ID 过滤，数据量继续扩大后还要评估候选集上限、分片、备份与重建时间。ModelGateway 的结构化输出与模型预算治理目前也比较轻。
