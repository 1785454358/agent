...
MySQL 适合保存作用域、类型、状态、版本、置信度、重要度和来源等权威结构化数据，也支持事务更新。Chroma 负责根据当前问题找到语义相近的候选。系统先用 MySQL 限定合法候选，再让 Chroma 在候选 ID 内做 TopK，最后回查 MySQL，因此向量索引无法绕过权限和生命周期规则。
...
项目需要处理中英文研究问题，本机已有 BAAI/bge-m3，可以离线生成统一的 1024 维向量，避免把记忆正文交给外部 embedding 服务。模型在 Worker 进程内懒加载一次，编码放到线程执行并用锁限制并发，避免阻塞事件循环和重复占用显存。
...
长期记忆自动清扫尚未调度，短期窗口按消息数而非精确 Token，ModelGateway 的结构化输出与模型预算治理还较轻，API 仍是固定身份的单租户。Chroma 使用单集合和候选 ID 过滤，数据量继续扩大后需要评估分片、备份与重建时间。
