...
flowchart TB
    ...
    MEM --> MYSQL
    MEM --> BGE[本地 BGE-M3]
    BGE --> CHROMA[(Chroma 语义索引)]
    CP -.本地模式.-> SQLITE[(SQLite)]
    EVIDENCE -.本地模式.-> SQLITE
```

讲图时从中间开始。顶层图管理公共生命周期，策略子图决定怎样研究，Gateway 管理外部调用，存储层管理可恢复事实。本地模式的 Checkpoint 和 Evidence 使用 SQLite，工具账本与结构化长期记忆只在进程内保存，Chroma 可以使用本地持久目录。API 重启后，残留向量没有权威候选记录，不能单独恢复长期记忆。分布式模式把权威记录放入 MySQL，Chroma 只保存由 BGE-M3 生成的长期记忆语义索引。
...
sequenceDiagram
    ...
    G->>G: 初始化 管理上下文 识别意图
    G->>M: 按意图召回长期记忆
    M->>M: 权威 Store 过滤 BGE 编码 Chroma TopK Store 回查
    alt 需要研究
        G->>P: ResearchInput
        P->>T: 搜索与抓取
        T->>E: 保存正文
...
flowchart LR
    ...
    LM --> C0[Local 进程内权威记录]
    LM --> C1[Distributed MySQL 权威 Preference]
    LM --> C2[Distributed MySQL 权威 Fact]
