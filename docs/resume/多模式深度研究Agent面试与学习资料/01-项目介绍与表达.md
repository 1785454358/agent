...
会话上下文保留近期原始消息，超过阈值后把较早消息压缩成结构化 `ConversationSummary`。长期记忆有独立写入门槛。用户明确要求记住时可以保存偏好，研究后的事实只有绑定 Evidence ID 才能写入。召回先由 MySQL 按作用域、类型、状态和有效期过滤，再由本地 BGE-M3 和 Chroma 在候选 ID 中做 TopK，命中结果回查 MySQL 后结合重要度、置信度和时效排序。Chroma 故障时退回确定性关键词检索。
