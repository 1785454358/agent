...
会话状态使用滑动窗口和结构化摘要压缩，长期记忆只保存明确偏好和有证据支持的事实。分布式模式下 MySQL 保存权威状态、Checkpoint、证据和执行账本，Redis Streams 负责任务投递与回收，Pub/Sub 唤醒 SSE。任务中断后，Checkpoint 恢复执行位置，工具账本复用已经完成的调用并重建预算。
...
会话上下文保留近期原始消息，超过阈值后把较早消息压缩成结构化 `ConversationSummary`。长期记忆有独立写入门槛。用户明确要求记住时可以保存偏好，研究后的事实只有绑定 Evidence ID 才能写入。Distributed 模式由 MySQL 按作用域、类型、状态和有效期过滤，再用 BGE-M3 和 Chroma 在候选 ID 中做 TopK，命中结果回查 MySQL 后结合重要度、置信度和时效排序。Local 模式复用同一召回流程，但权威记录暂存在进程内 Memory Store，API 重启后不会保留这些结构化记忆。Chroma 故障时，两种模式都会退回确定性关键词检索。

分布式模式使用 MySQL 保存运行记录、事件、Checkpoint、Evidence、长期记忆、线程租约和工具执行账本，Chroma 保存长期记忆语义索引，Redis Streams 负责 at-least-once 任务投递，Pub/Sub 只负责唤醒 SSE 读取。Worker 通过运行租约和线程租约避免并发写同一任务或会话。崩溃恢复时，Checkpoint 决定从哪个图节点继续，执行账本避免重复使用已完成的工具结果，并重建已消耗预算。
...
- 当前非真实测试覆盖策略、工具、记忆、持久化、分布式与恢复链路。投递简历前应重新运行并记录数量。
